...
function formatTime(minutes, seconds) {
  const m1 = minutes.toString().padStart(2, '0');
  const s1 = seconds.toString().padStart(2, '0');
  return m1.concat(':', s1);
}
...
function isPalindrome(str) {
  const str1 = str.toLowerCase().replace(/\W/g, '').split('');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    if (words[i].length > mlenght) {
      mlenght = words[i].length;
      mWord = words[i];
    }
  }
  return mWord;
...
function reverseWords(str) {
  ...
  const newAr = [];
  for (let i = 0; i < str1.length; i += 1) {
    newAr.push(str1[i].split('').reverse().join(''));
  }
  return newAr.join(' ');
}
...
function extractNameFromTemplate(value) {
  ...
  return arr.slice(1).join(' ').replace('!', '');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    let UTFcode = array[i].charCodeAt();
    if ((UTFcode >= 65 && UTFcode <= 90) || (UTFcode >= 97 && UTFcode <= 122)) {
      UTFcode += 13;
      if ((UTFcode > 90 && UTFcode < 110) || UTFcode > 122) UTFcode -= 26;
      arrCode.push(String.fromCharCode(UTFcode));
    } else {
      arrCode.push(String.fromCharCode(UTFcode));
    }
  }
